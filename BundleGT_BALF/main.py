...
from models.BundleGT import BundleGT, BundleGT_BALF
import eval_model
...
def get_cmd():
    ...
    parser.add_argument("-d", "--dataset", default="Meal", type=str, help="which dataset to use, options: NetEase, Youshu, iFashion")
    parser.add_argument("-m", "--model", default="BundleGT_BALF", type=str, help="which model to use, options: BundleGT, BundleGT_BALF")
    ...
    parser.add_argument( "--num_trans_layers", default=3, type=int, help="the number of transformer layers")
    ...
    parser.add_argument( "--num_token", default=70, type=int, help="(0, Maximum], 0 means keep the whole sequence")
...
def main():
    ...
    if conf['model'] == 'BundleGT':
        ...
        print("BundleGT loaded!")
    elif conf['model'] == 'BundleGT_BALF':
        model = BundleGT_BALF(conf, dataset.graphs).to(device)
        print("BundleGT_BALF loaded!")
    else:
    ...
    # resume_path = os.path.join(checkpoint_model_path, 'checkpoint_latest.pth')
    start_epoch = 0
    ...
    for epoch in range(start_epoch, conf['epochs']):
        ...
        if (epoch + 1) % 10 == 0:  # 每10个epoch保存一次
            ...
            torch.save(checkpoint_data, os.path.join(checkpoint_model_path, 'checkpoint_latest.pth'))
