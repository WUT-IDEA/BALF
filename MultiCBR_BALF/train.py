...
from models.MultiCBR_BALF import MultiCBR_BALF
import eval_model
...
def get_cmd():
    ...
    parser.add_argument("-g", "--gpu", default="2", type=str, help="which gpu to use")
    parser.add_argument("-d", "--dataset", default="Youshu", type=str, help="which dataset to use, options: NetEase, iFashion")
    parser.add_argument("-m", "--model", default="MultiCBR_BALF", type=str, help="which model to use, options: MultiCBR, MultiCBR_BALF")
...
def main():
    ...
    assert paras["model"] in ["MultiCBR", "MultiCBR_BALF"], "Pls select models from: MultiCBR, MultiCBR_BALF"
    ...
    for lr, l2_reg, UB_ratio, UI_ratio, BI_ratio, embedding_size, num_layers, c_lambda, c_temp in \
        ...
        if conf['model'] == 'MultiCBR':
            ...
            print("MultiCBR loaded!")
        elif conf['model'] == 'MultiCBR_BALF':
            model = MultiCBR_BALF(conf, dataset.graphs).to(device)
            print("MultiCBR_BALF loaded!")
        else:
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                run.add_scalar("reg_loss", reg_loss_scalar)
        ...
        eval_model.eval(model, device, dataset.test_loader, Test=True)
