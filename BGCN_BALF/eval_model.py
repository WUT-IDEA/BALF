...
def eval(model, device, dataloader, Test=False):
    ...
    popular_bundles, unpopular_bundles = group_bundles_by_popularity(model.ub_graph)  # train
...
def group_bundles_by_popularity(ub_graph, num_groups=3):
    """
    将bundle按流行度分num_groups组，每组的bundle popularity总和相同，第一组的bundle为popular bundle，其余为unpopular bundle
    """
    bundle_popularity = torch.as_tensor(ub_graph.sum(axis=0)).flatten()
