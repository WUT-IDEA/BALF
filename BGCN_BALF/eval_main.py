...
import setproctitle
import dataset
from model import BGCN, BGCN_Info, BGCN_BALF, BGCN_BALF_Info
...
def main():
    ...
    for DIR in CONFIG['test']:
        ...
        for i in range(len(d)):
            ...
            elif CONFIG['model'] == 'BGCN_BALF':
                graph = [ub_graph, ui_graph, bi_graph]
                info = BGCN_BALF_Info(64, dd['embed_L2_norm'], dd['mess_dropout'], dd['node_dropout'], 2)
                model = BGCN_BALF(info, assist_data, graph, device, pretrain=None).to(device)

            assert model.__class__.__name__ == CONFIG['model']

            print(f'model:{CONFIG["model"]}, dataset:{CONFIG["dataset_name"]}')
            ...
            eval_model.eval(model, device, test_loader, Test=True)
