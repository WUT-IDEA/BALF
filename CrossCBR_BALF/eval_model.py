...
def eval(model, device, dataloader, epoch=None, Test=False):
    model.eval()

    # item median定义pop unpop bundle
...
def group_bundles_by_popularity(ub_graph, ui_graph, bi_graph, num_groups=3):
    """
    bundle包含item的median作为popularity值
    ...
    # bundle_popularity = torch.as_tensor(bi_graph @ item_popularity)
