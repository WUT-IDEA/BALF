...
from models.CrossCBR_BALF import CrossCBR_BALF
import eval_model
import csv
...
def get_cmd():
    ...
    parser.add_argument("-g", "--gpu", default="1", type=str, help="which gpu to use")
    parser.add_argument("-d", "--dataset", default="Youshu", type=str, help="which dataset to use, options: NetEase, Youshu, iFashion, Meal")
    parser.add_argument("-m", "--model", default="CrossCBR", type=str, help="which model to use, options: CrossCBR, CrossCBR_BALF")
...
def main():
    ...
    assert paras["model"] in ["CrossCBR", "CrossCBR_BALF"], "Pls select models from: CrossCBR, CrossCBR_BALF"
    ...
    for lr, l2_reg, item_level_ratio, bundle_level_ratio, bundle_agg_ratio, embedding_size, num_layers, c_lambda, c_temp in \
        ...
        fairness_metrics_path = f"./results_OOD/{conf['dataset']}/{conf['model']}/{item_level_ratio}_{bundle_level_ratio}_{bundle_agg_ratio}"
        if not os.path.isdir(fairness_metrics_path):
            os.makedirs(fairness_metrics_path)


        # model
        if conf['model'] == 'CrossCBR':
            model = CrossCBR(conf, dataset.graphs).to(device)
            print("CrossCBR loaded!")
        elif conf['model'] == 'CrossCBR_BALF':
            model = CrossCBR_BALF(conf, dataset.graphs).to(device)
            print("CrossCBR_BALF loaded!")
        else:
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                if conf["aug_type"] == "ED" and (batch_anchor+1) % ed_interval_bs == 0:
                    ED_drop = True
                bpr_loss, c_loss, reg = model(batch, ED_drop=ED_drop)
                loss = bpr_loss + c_loss + reg
                loss.backward()
                optimizer.step()
                ...
                run.add_scalar("reg", reg_loss_scalar, batch_anchor)

                pbar.set_description("epoch: %d, loss: %.4f, bpr_loss: %.4f, c_loss: %.4f, reg_loss: %.4f" %(epoch, loss_scalar, bpr_loss_scalar, c_loss_scalar, reg_loss_scalar))
            ...
            # # --- Fairness metrics per epoch ---
            # metrics_epoch = fairness_metrics(model, dataset, device)

            # # --- append csv 文件 ---
            # csv_path = os.path.join(fairness_metrics_path, "epoch_wise_fairness_metrics.csv")
            # is_new = not os.path.exists(csv_path)

            # metrics_epoch["epoch"] = epoch + 1
            # is_new = (epoch == 0) # 在第一个 epoch (epoch 0) 时创建文件

            # with open(csv_path, "a", newline="") as f:
            #     writer = csv.DictWriter(f, fieldnames=["epoch", "entropy", "hhi", "AF"])
            #     if is_new:
            #         writer.writeheader()
            #     writer.writerow(metrics_epoch)
        eval_model.eval(model, device, dataset.test_loader, epoch=epoch, Test=True)

        

def fairness_metrics(model, dataset, device):
    model.eval()
    rs = model.propagate(test=True)

    num_bundles = dataset.num_bundles
    num_items = dataset.num_items

    bundle_pop = torch.zeros(num_bundles, device=device)

    b_i_graph = dataset.graphs[2]   # scipy.sparse.csr_matrix

    # === 遍历 test loader，仅统计 bundle popularity ===
    for users, _, train_mask_u_b in dataset.test_loader:
        users = users.to(device)
        train_mask_u_b = train_mask_u_b.to(device)

        pred_b = model.evaluate(rs, users)
        pred_b -= 1e8 * train_mask_u_b

        top100 = torch.topk(pred_b, 100).indices  # (batch,100)

        # 统计每个 bundle 被选入 top100 的次数
        bundle_pop.index_add_(
            0,
            top100.reshape(-1),
            torch.ones(top100.numel(), device=device)
        )

    # === item popularity:  使用矩阵运算 item_pop = B^T * bundle_pop ===
    B = b_i_graph
    B_coo = B.tocoo()

    indices = torch.tensor(
        [B_coo.row, B_coo.col], device=device
    )
    values = torch.tensor(B_coo.data, dtype=torch.float32, device=device)

    B_sparse = torch.sparse_coo_tensor(indices, values, size=B.shape, device=device)

    # item_pop[i] = sum_{b contains i}(bundle_pop[b])
    item_pop = torch.sparse.mm(B_sparse.transpose(0,1), bundle_pop.unsqueeze(1)).squeeze()

    # === Entropy + HHI（基于 bundle_pop）===
    total = bundle_pop.sum() + 1e-8
    p = bundle_pop / total
    ...
    # === AF 计算 ===
    # 需要行归一化 B_norm（bundle->item），然后 Br_i = B_norm * r_i
    row_sum = torch.tensor(B.sum(axis=1)).flatten().to(device).float() + 1e-8
    values_norm = values / row_sum[B_coo.row]
    B_norm = torch.sparse_coo_tensor(indices, values_norm, size=B.shape, device=device)

    Br_i = torch.sparse.mm(B_norm, item_pop.unsqueeze(1)).squeeze()

    AF = bundle_pop.norm().item() / (Br_i.norm().item() + 1e-8)
...
def write_log(run, log_path, topk, step, metrics):
    ...
    if topk == 20:  # 只对 Top_20 进行标红
        val_str = f"\033[91m{val_str}\033[0m"
        test_str = f"\033[91m{test_str}\033[0m"
...
def log_metrics(conf, model, metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor, best_metrics, best_perform, best_epoch):
    ...
    if metrics["test"]["recall"][topk_] > best_metrics["test"]["recall"][topk_] and metrics["test"]["ndcg"][topk_] > best_metrics["test"]["ndcg"][topk_]:
        ...
        for topk in conf['topk']:
            ...
            if topk == 20:  # 只对 Top_20 进行标蓝
                best_perform["val"][topk] = f"\033[94m{best_perform['val'][topk]}\033[0m"
                best_perform["test"][topk] = f"\033[94m{best_perform['test'][topk]}\033[0m"
